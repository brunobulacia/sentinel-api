...
export class ScanHistoryService {
  ...
  async stats(userId: string) {
    ...
    return all.map((ex) => ({
      ...
      high: ex.vulnerabilities.filter((v) => v.criticality === Criticality.HIGH)
        .length,
      medium: ex.vulnerabilities.filter(
        (v) => v.criticality === Criticality.MEDIUM,
      ).length,
      low: ex.vulnerabilities.filter((v) => v.criticality === Criticality.LOW)
        .length,
    }));
  }
  ...
  async compare(id1: string, id2: string) {
    ...
    return {
      ...
      resolvedInScan2: vulns1
        .filter((v) => !names2.has(v.name))
        .map((v) => v.name),
