...
export class ScanExecutionService {
  ...
  async create(dto: CreateScanExecutionDto, userId: string) {
    ...
    this.runScan(execution.id, config.targetUrl, config.depth).catch(() => null);
    return execution;
  }

  private async runScan(executionId: string, targetUrl: string, depth: string): Promise<void> {
    ...
    try {
      ...
      if (vulnerabilities.length > 0) {
        await this.prisma.vulnerability.createMany({
          data: vulnerabilities.map((v) => ({ ...v, scanExecutionId: executionId })),
        });
      }
